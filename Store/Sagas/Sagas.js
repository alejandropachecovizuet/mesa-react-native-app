...
const registroEnFirebase = values =>
  autenticacion
    ...
    .then(success => autenticacion.currentUser);
...
//    const imagen = yield select(state => state.reducerImagenSignUp);
  //  console.log(imagen);
    ...
    const fotoURL = 'https://res.cloudinary.com/dpykmlkbz/image/upload/v1527288822/sample.jpg';
    const registro = yield call(registroEnFirebase, values.datos);
    console.log('registro->' + registro);
    const { uid } = registro; 
    const { datos: { correo, nombre } } = values;
    // uid, email, nombre
    console.log('uid:' + uid + ',correo:' + correo + ',nombre:' + nombre + ",fotoURL:" + fotoURL);
...
function* sagaSubirPublicacion(values) {
  try {
    //throw new Error('La publicación no se realizo');
  ...
  //  const resultadoImagen = yield call(registroFotoCloudinary, imagen);
    //console.log("resultadoImagen->" + resultadoImagen);
    ...
    const width=100;
    const height=101;
    const secure_url='https://res.cloudinary.com/dpykmlkbz/image/upload/v1527288822/sample.jpg';
    const parametrosImagen = { width, height, secure_url, uid };
    console.log(' parametrosImagen->' +  parametrosImagen);
